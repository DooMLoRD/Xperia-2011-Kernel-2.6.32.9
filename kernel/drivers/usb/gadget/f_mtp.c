...
requeue_req:
	...
	if (dev->state == STATE_CANCELED)
		...
		DBG(cdev, "mtp_read == DEVICE RESET in done block\n");
		r = -ECONNRESET;
	}
	...
	while (count > 0 || write_req) {
		...
		if (read_req) {
			...
			if (dev->state == STATE_RESET) {
				...
				r = -ECONNRESET;
				if (!dev->rx_done) {
					DBG(cdev, "usb_ep_dequeue DEVICE RESET\n");
					usb_ep_dequeue(dev->ep_out, read_req);
				}
				break;
			}
...
static long mtp_ioctl(struct file *fp, unsigned code, unsigned long value)
{
	...
	case MTP_SEND_FILE_WITH_HEADER:
	{
		...
		if (dev->state == STATE_RESET) {
				...
				DBG(dev->cdev, "report reset to user space... mtp_ioctl... DEVICE RESET");
				dev->state = STATE_READY;
				spin_unlock_irq(&dev->lock);
				ret = -ECONNRESET;
				goto out;
...
fail:
	...
	if (dev->state == STATE_CANCELED)
		...
		DBG(dev->cdev, "failed case: mtp_ioctl.. DEVICE RESET");
		ret = -ECONNRESET;
	}
	else if (dev->state != STATE_OFFLINE)
		dev->state = STATE_READY;
	...
	if ((ctrl->bRequestType & USB_TYPE_MASK) == USB_TYPE_CLASS) {
		...
		if (ctrl->bRequest == MTP_REQ_CANCEL && w_index == 0
			...
			if (dev->state == STATE_CANCELED)
				...
				DBG(cdev, "Device goest to ready state from reset state... DEVICE RESET\n");
			}
			else
				status->wCode =
					__cpu_to_le16(MTP_RESPONSE_OK);
